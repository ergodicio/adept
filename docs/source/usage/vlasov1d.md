...
$$
\frac{\partial f}{\partial t} + v \frac{\partial f}{\partial x} + E \frac{\partial f}{\partial v} = C_{ee}(f) + C_{K}(f)
$$
...
$$
C_{ee}(f) = \nu_{ee} \frac{\partial}{\partial v} \left( v f + v_{th}^2 \partial_v f \right)
$$
...
What is `nu_ee`? It will modify the dynamics of the problem, possibly substantially depending on the distribution function dynamics. The envelope for this can also be specified in the same way as the driver.
