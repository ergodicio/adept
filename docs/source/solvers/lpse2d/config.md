...
| `stride` | int | (default `1`; `spectral` or `fd`, not `explicit`) Advance the IAW every `stride` EPW steps with `dt_iaw = stride * grid.dt` (LPSE steps its IAW solver less often than the EPW; the deck translator writes LPSE's ratio) |
