...
| `boundary_profile` | string | (optional, default `exp`, as LPSE) Shape of the absorbing layers. `tanh` is adept's pre-port envelope (the MATLAB v11 prototype also uses the exponential profile; LPSE refuses tanh) (rise `boundary_width / 5`). `exp` is the original LPSE profile (`absorbingBoundaries.cpp`): rate `boundary_max_rate * (e^{lambda s/L} - 1) / (e^lambda - 1)` with `s` the distance into the layer measured from the edge cell (so the edge cell carries the full rate and the layer spans `boundary_width / dx` cells); with both axes absorbing the two rates combine by `max`, as in LPSE |
...
| `dealias` | string | Shape of the anti-aliasing mask: `rectangular` (default, LPSE), `isotropic` or `shifted-band` (LPSE `grid.antiAliasing.range`: the outer `1 - low_pass_filter` of each k axis is zeroed with LPSE's index rounding -- `Lpse::setupAntiAliasingRange`, float32 `int(N (1 -/+ aa)/2)` rounded down to even and made symmetric about k = 0, `x0 < l < x1` zeroed, which keeps one or two modes more than `|k| < low_pass_filter k_Nyquist`; a grid LPSE would refuse as asymmetric raises; the deck translator uses it with LPSE's range, `0.3334` when the deck omits the key, ParameterManager.cpp:241) |
...
| Value | Mask |
|-------|------|
| `isotropic` | `|k| < low_pass_filter * kmax` only. Alias-free only if `low_pass_filter * kmax + k0 <= kmax`, which is not checked. |
...
| `swelling` | string | (default `constant`) Field swelling of the static (prescribed) pump: `constant` multiplies it by `(1 - n_env)^(-1/4)`, LPSE's default (`LightSolver::computeStaticE0_fft` / `_xSpace`); `local` by the local `(1 - n)^(-1/4)`, zero from 0.9999 n_c and at most 10 (LPSE `laser.static.useSpatiallyVaryingFieldSwelling`, the MATLAB prototype's convention, and adept's before the C++ standardisation). The deck translator maps LPSE's flag |
...
| `turn_on_time` | string | (pump depletion only, optional) Gaussian turn-on time of the injector. Default `30fs` (LPSE `laser.evolution.riseTime`) |
...
| `turn_on_time` | string | Ramp-up time of the injector, `1 - exp(-(t / turn_on_time)^2)` (default `30fs`, LPSE's; the deck translator uses LPSE's `raman.evolution.riseTime`, default 30 fs) |
...
| `suppress_sources_at_injectors` | bool | (default `false`; LPSE's default is `true` and the deck translator sets it) zero the EPW and IAW sources across the pump injector rows (with `pump_depletion`) and the seed injector rows: the two FD source rows, or the Gaussian source's width in cells for the spectral solver (LPSE `suppressSourcesAtInjectors`, `ssWidth`). Independently of it, each light field's own coupling sources (SRS, depletion, TPD depletion, the combined solver's unified source and depletion) are always zeroed at the *partner* field's injector rows -- the Raman light's at the pump's, the pump's at the seed's -- as LPSE `LightSolver::calculateSources` does; `suppress_sources_in_absorbers` zeroes them inside the field's own layer (`boundary_width` / `raman_boundary_width`), and the IAW drive inside the IAW's layer. The exact exchange of `coupling: rotation` is switched off where either field's source is zeroed |
...
| `srs_k_filter` | bool | (default `false`, LPSE `lw.kFilter.enable`) apply a high-k filter to the light fields entering the SRS source, cutting at `srs_k_filter_scale` times the Raman wavenumber at the minimum box density (LPSE `lw.kFilter.enable`, which is *off* by default there; MATLAB `isSuppressHighKSource`). The cutoff assumes the scattered light sits exactly at the envelope frequency, so in a box detuned from the envelope density it can remove the resonant Raman mode entirely (no SRS growth at all at 0.2 nc with envelope density 0.25); translated LPSE decks turn it off |
...
| `damping.collisions` | float | Collisional damping rate in `1/ps` (default `0`, LPSE's): on `n` as `(1 - nu dt)` in the explicit solver, on `div v` as `exp(-2 nu dt)` in the spectral one (LPSE) |
| `damping.landau` | float | Dimensionless coefficient in `gamma_iaw(k) = landau * cs * |k|` (default `0`, LPSE's); the velocity-divergence equation is damped at `2 gamma_iaw` |
...
| `max_wavenumber` | float or null | (optional) LPSE `iaw.maxWavenumber` (`IawSolver.cpp:232-234`): hard cap `\|k\| < max_wavenumber * k0` on the IAW band. The IAW keeps the anti-aliased band otherwise -- `terms.epw.max_wavenumber` does not act on it. The deck translator passes the deck's value |
...
| `source_window` | object | (optional) LPSE `iaw.restrictSourceRange`, the same window as `terms.epw.source_window`; it multiplies the ponderomotive drive **squared** (LPSE `getPonderomotivePotential`). The `spectral` and `fd` solvers apply it, with the injector-row and absorber suppression, to `E2 = k^2 PP` after the Laplacian as LPSE (`suppressSourceInSpecificRegions(pp, false, true)`); the `explicit` (MATLAB) solver to the potential |
...
`terms.iaw.thermal_filamentation: {laser: bool, raman: bool, lw: bool, nonlocal: bool, conductivity_multiplier: 1.0}` adds LPSE's thermal-filamentation source (`ZakharovSolver::getThermalFilamentationSource`) to the ion velocity-divergence equation: inverse-bremsstrahlung heating by the spatially varying part of each enabled wave's intensity, balanced by Spitzer heat conduction, drives the flow through the electron pressure, `d(div v)/dt += K_w (n/n_c,w)^2 X_w` for light (`K_lw (n/n_env) X_lw` for the EPW) with `X_w = |E_w|^2 - <|E_w|^2>` (the average weighted by the IAW source window), `K_w = Z 2 nu_w / (8 pi m_i kappa')`, `nu_w` the wave's absorption rate at its critical density (requires `terms.light.absorption`; EPW: `nu_coll`, requires `terms.epw.damping.collisions`), `kappa'` the Spitzer conductivity with LPSE's electron-ion Coulomb logarithm at the envelope density (times `conductivity_multiplier`) and `n = n_b (1 + Nelf)` the total density. `nonlocal: true` adds LPSE's `n^(2/3) Lambda^(4/3) F^-1[|k|^(4/3) F[sum_w K_w X_w / n_c,w^2]]` (cgs, the IAW band only; `Lambda = 30 (k_B T_e)^2 / (4 pi e^4 sqrt(Z+1) ln Lambda_ee)`), `(k lambda_nl)^(4/3)` times the local term at uniform density. The local term carries the IAW source mask squared, the non-local one the window inside the transform and the mask after it, as LPSE; LPSE leaves the pre-sum's modes outside the IAW band unmultiplied (in its own units), which are dropped here. With the combined solver the Raman term is the whole combined field at `wp0` and `lw` is ignored. The overall normalisation is adept's own (derived from the heat and momentum equations).
...
`adept._lpse2d.threshold.find_threshold` bisects the pump intensity between a stable and an unstable bracket on the fitted EPW growth rate. `find_threshold_lpse` is LPSE's own `absoluteThreshold` search (`AbsoluteThreshold.cpp`): from the configuration's intensity it steps by `dI_fract` of it away from the first run's side until the gain criterion flips, then `n_iter` halving steps towards the flip. The criterion is LPSE's `checkIsAboveThreshold` sample by sample: `A` is `max |rho|` in x-space (`rho = div E`, the default series' `max_rho`; `max_nelf` for a deck without Langmuir waves), samples before `noise_time_range[0]` are ignored, `A_noise` is the running mean over `[noise_time_range[0], noise_time_range[1])`, and the run is above threshold at the first sample with `ln(A / A_noise) > gain` (LPSE ends the run there). The deck translator writes `absoluteThreshold.{gain, dI_fract, numIterations, noiseTimeRange}` into a `threshold` block that `find_threshold_lpse` reads (test_016); the defaults are LPSE's (`gain` 23.026, `dI_fract` 0.3333, 5 iterations, 0.01-0.1 ps), and `statistic` names the series quantity.
