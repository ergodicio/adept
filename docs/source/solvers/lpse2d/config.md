...
| `dealias` | string | Shape of the anti-aliasing mask: `isotropic` (default), `shifted-band` or `rectangular` (LPSE `grid.antiAliasing.range`: the outer `1 - low_pass_filter` of each k axis is zeroed with LPSE's index rounding -- `Lpse::setupAntiAliasingRange`, float32 `int(N (1 -/+ aa)/2)` rounded down to even and made symmetric about k = 0, `x0 < l < x1` zeroed, which keeps one or two modes more than `|k| < low_pass_filter k_Nyquist`; a grid LPSE would refuse as asymmetric raises; the deck translator uses it with LPSE's range, `0.3334` when the deck omits the key, ParameterManager.cpp:241) |
