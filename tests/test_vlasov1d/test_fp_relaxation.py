...
#  research@ergodic.io
"""Cartesian Fokker-Planck relaxation tests (LenardBernstein, Dougherty)."""
...
PROBLEMS = [
    ...
    {"ic_fn": partial(shifted_maxwellian, v_shift=1.8, T=0.162), "extra_checks": "momentum", "equilibrium": True},
    {"ic_fn": partial(shifted_maxwellian, v_shift=1.0, T=1.0), "extra_checks": "momentum", "equilibrium": True},
]
...
def test_fp_relaxation(problem, slow):
    run_relaxation_sweep_and_assert(
        "cartesian", MODELS, EXPERIMENT, problem, slow, SLOW_EXTRA_COMBOS, NV, VMAX, TEMPERATURE_TOL
    )
