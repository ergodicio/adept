...
def get_field_save_func(cfg):
    if {"t"} == set(cfg["save"]["fields"].keys()):
        ...
        def fields_save_func(t, y, args):
            temp = {"n": _calc_moment_(y["ion"]), "v": _calc_moment_(y["ion"] * cfg["grid"]["v"][None, :])}
            v_m_vbar = cfg["grid"]["v"][None, :] - temp["v"][:, None]
            temp["p"] = _calc_moment_(y["ion"] * v_m_vbar**2.0)
            temp["q"] = _calc_moment_(y["ion"] * v_m_vbar**3.0)
            temp["-flogf"] = _calc_moment_(y["ion"] * jnp.log(jnp.abs(y["ion"])))
            temp["f^2"] = _calc_moment_(y["ion"] * y["ion"])
            temp["e"] = y["e"]
            temp["de"] = y["de"]
            temp["a"] = y["a"]
...
def get_save_quantities(cfg: dict) -> dict:
    ...
    for save_type, save_config in cfg["save"].items():  # this can be fields or electron or diags?
        ...
        elif save_type.casefold() in ["ion"]:
...
def get_default_save_func(cfg):
    ...
    def save(t, y, args):
        scalars = {
            "mean_P": _calc_mean_moment_(y["ion"] * v**2.0),
            "mean_j": _calc_mean_moment_(y["ion"] * v),
            "mean_n": _calc_mean_moment_(y["ion"]),
            "mean_q": _calc_mean_moment_(y["ion"] * v**3.0),
            "mean_-flogf": _calc_mean_moment_(-jnp.log(jnp.abs(y["ion"])) * jnp.abs(y["ion"])),
            "mean_f2": _calc_mean_moment_(y["ion"] * y["ion"]),
