...
class FieldSolver:
    ...
    def compute_charge(self, f):
        return self.moment_x(self.moment_y(f))

    def compute_jx(self, f):
        return self.moment_x(self.moment_y(self.vx * f)) * self.kx_mask

    def compute_jy(self, f):
        return self.moment_x(self.moment_y(self.vy * f)) * self.ky_mask
    ...
    def hampere_e1(self, exk, fxy, dt):
        ...
        return exk + self.one_over_ikx * self.moment_x(
            self.moment_y(fk * (jnp.exp(-1j * self.kx[..., None, None] * dt * self.vx) - 1))
        )

    def hampere_e2(self, eyk, fxy, dt):
        fk = jnp.fft.fft2(fxy, axes=(0, 1))
        return eyk + self.one_over_iky * self.moment_x(
            self.moment_y(fk * (jnp.exp(-1j * self.ky[..., None, None] * dt * self.vy) - 1))
        )
