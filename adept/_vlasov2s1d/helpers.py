...
from adept._vlasov2s1d.storage import store_f, store_fields, store_diags
...
def _initialize_distribution_(
    ...
    single_dist = -(np.power(np.abs((vax[None, :] - v0) / alpha / np.sqrt(T0)), m) * mass)
...
def post_process(result: Solution, cfg: dict, td: str, args: dict):
    ...
    for k in result.ys.keys():
        if k.startswith("field"):
            ...
            for nm, srs in scalars_xr.items():
                ...
                # Handle log plotting more carefully
                abs_srs = np.abs(srs)
                non_zero_mask = abs_srs > 1e-16
                if non_zero_mask.any():
                    log_srs = np.log10(abs_srs.where(non_zero_mask))
                    log_srs.plot(ax=ax[1])
                    ax[1].set_ylabel("$log_{10}$(|" + nm + "|)")
                else:
                    ax[1].text(0.5, 0.5, "All values zero", ha="center", va="center", transform=ax[1].transAxes)
                    ax[1].set_ylabel("$log_{10}$(|" + nm + "|)")
    ...
    # Store diagnostics if any exist
    diags_keys = [k for k in result.ys.keys() if k.startswith("diag-")]
    if diags_keys:
        diags_xr = store_diags(cfg, result.ts, td, result.ys)
    else:
        diags_xr = None

    # Plot distribution functions for both species
    if f_xr is not None:
        os.makedirs(os.path.join(td, "plots", "distributions"), exist_ok=True)

        # Plot electron distribution if it exists
        if "electron" in f_xr:
            electron_dist = f_xr["electron"]
            t_skip = max(1, electron_dist.coords["t"].size // 8)
            tslice = slice(0, -1, t_skip)

            electron_dist[tslice].plot(col="t", col_wrap=4, x="v", y="x")
            plt.savefig(os.path.join(td, "plots", "distributions", "electron_dist.png"), bbox_inches="tight")
            plt.close()

        # Plot ion distribution if it exists
        if "ion" in f_xr:
            ion_dist = f_xr["ion"]
            t_skip = max(1, ion_dist.coords["t"].size // 8)
            tslice = slice(0, -1, t_skip)

            ion_dist[tslice].plot(col="t", col_wrap=4, x="v_i", y="x")
            plt.savefig(os.path.join(td, "plots", "distributions", "ion_dist.png"), bbox_inches="tight")
            plt.close()

    mlflow.log_metrics({"postprocess_time_min": round((time() - t0) / 60, 3)})

    return {"fields": fields_xr, "dists": f_xr, "scalars": scalars_xr, "diagnostics": diags_xr}
