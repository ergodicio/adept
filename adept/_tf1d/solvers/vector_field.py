...
class VF(eqx.Module):
    ...
    def __init__(self, cfg):
        ...
        self.driver_names = tuple(cfg["drivers"]["ex"])
        self.nx = cfg["grid"]["nx"]
        ...
        self.push_driver = cast(pushers.Driver, pushers.Driver(cfg["grid"]["x"]))
        # if "ey" in self.cfg["drivers"]:
        #     self.wave_solver = pushers.WaveSolver(cfg["grid"]["c"], cfg["grid"]["dx"], cfg["grid"]["dt"])
        self.poisson_solver = cast(pushers.PoissonSolver, pushers.PoissonSolver(cfg["grid"]["one_over_kx"]))
    ...
    def __call__(self, t: float, y: dict, args: dict):
        ...
        e = self.poisson_solver(
            self.physics["ion"]["charge"] * y["ion"]["n"] + self.physics["electron"]["charge"] * y["electron"]["n"]
        )
        ed = 0.0

        for p_ind in self.driver_names:
        ...
        for species_name in ["ion", "electron"]:
            ...
            if self.physics[species_name]["is_on"]:
                q_over_m = self.physics[species_name]["charge"] / self.physics[species_name]["mass"]
                p_over_m = p / self.physics[species_name]["mass"]
                ...
                dstate_dt[species_name]["n"] = jnp.zeros(self.nx)
                dstate_dt[species_name]["u"] = jnp.zeros(self.nx)
                dstate_dt[species_name]["p"] = jnp.zeros(self.nx)

            if self.physics[species_name]["trapping"]["is_on"]:
                dstate_dt[species_name]["delta"] = self.pusher_dict[species_name]["particle_trapper"](e, delta, args)
            else:
                dstate_dt[species_name]["delta"] = jnp.zeros(self.nx)
